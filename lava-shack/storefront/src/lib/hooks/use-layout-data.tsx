...
const fetchFeaturedProducts = async (
  ...
  const scores = await medusaClient.client
    .request("GET", `/store/product-reviews?productIds=${productIds.join(",")}`)
    .then(({ scores }) => scores)
    .catch((_) => [])
  ...
  return products
    ...
    .map((p) => {
      ...
      const score = scores.find((s: any) => s.product_id === p.id)
      ...
      return {
        ...
        review: { score: score?.score ?? 0, amount: score?.amount ?? 0 },
      }
    })
}
...
export const useFeaturedProductsQuery = (
  ...
  const queryResults = useQuery(
    ["layout_featured_products", cart?.id, cart?.region, maxProduct],
    () =>
      fetchFeaturedProducts(cart?.id!, cart?.region!, maxProduct, collections),
    {
