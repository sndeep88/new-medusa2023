...
export const StoreProvider = ({ children }: StoreProps) => {
  ...
  const ensureRegion = (region: Region, countryCode?: string | null) => {
    if (!IS_SERVER) {
      const us = region.countries.find((c) => c.iso_2 === "us")
      const { regionId, countryCode: defaultCountryCode } = getRegion() || {
        regionId: region.id,
        countryCode:
          us?.iso_2 ?? region.countries.sort((a, b) => a.id - b.id)[0].iso_2,
      }
  ...
  const resetCart = async () => {
    ...
    let cartId = window.localStorage.getItem("savedCart")
  ...
  async function createBuynowCart() {
    setIsQuickCheckout(true)
    // ressave current cart

    // @ts-ignore
    if (cart?.items?.length > 0) {
      const cartId = getCart()
      if (cartId) {
        window.localStorage.setItem("savedCart", cartId)
      }
      await createNewCart()
      return
    }
