...
interface CheckoutContext {
  ...
  updateDiscount: (data: { discount_code: string }) => void
  setCart: (cart: Omit<Cart, "refundable_amount" | "refunded_total">) => void
...
export const CheckoutProvider = ({
  ...
  const {
    ...
    updateCart: { mutateAsync: updateCart, isLoading: updatingCart },
  } = useCart()
  ...
  const cardMethods = useForm<CardInfomation>({
    defaultValues: {
      card_number: "",
      card_expMon: new Date().getMonth().toString().padStart(2, "0"),
  ...
  const {
    mutateAsync: deletepaymentSession,
    isLoading: removingPaymentSession,
  } = useDeletePaymentSession(cart?.id!)
  ...
  /**
   * Update cart discount code
   */
  const updateDiscount = async (data: { discount_code: string }) => {
    if (cart) {
  ...
  const initPayment = async () => {
    ...
    if (
      ...
      const paymentSessions = await createPaymentSessions(cart.id)
      // console.log({ paymentSessions })
      if (!paymentSessions) {
        // console.log("rerun")
        t = setTimeout(initPayment, 500)
      } else {
        setCart(paymentSessions)
   ...
   */
  const [showPaymentSelect, setShowPaymentSelect] = useState<{
    show: boolean
    error?: string
  }>({
    show: false,
  ...
  const setAddresses = async (data: CheckoutFormValues) => {
    ...
    methods.setValue(
      "billing_address",
      payload.billing_address as AddressValues
    )
