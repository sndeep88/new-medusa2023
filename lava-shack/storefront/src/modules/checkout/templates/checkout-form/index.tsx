...
const CheckoutForm = ({ storeData }: { storeData?: any }) => {
  const { cart, handleSubmit, getValues, setAddresses } = useCheckout()
  ...
  // useEffect(() => {
  //   console.log({ type: "sendEmail", cart })
  // }, [cart])
  ...
  return (
    <div className="left-box-pd">
        ...
        <Link href="/">
          <a className="navbar-brand">
            {storeData && storeData.logo ? (
              <span className="relative block h-[75px] aspect-auto">
                <Image
                  src={storeData.logo}
                  objectFit="contain"
                  layout="fill"
              ...
              <strong>{storeData?.store_name}</strong>
            )}
          </a>
        </Link>
      </div>
      ...
      <div className="checkout-footer mt-0 mt-md-4">
        <p className="mb-2">
          All charges will have a variation on how they appear on your credit card statement.
        </p>
        ...
        <hr />
        <div>
          <small>© 2023 {storeData?.store_name}. All Rights Reserved.</small>
        </div>
      </div>
    </div>
  )
