...
import NewDescription from "./add-description"
...
const NewProduct = ({ onClose }: Props) => {
  ...
  const { mutate, mutateAsync } = useAdminCreateProduct()
  ...
  const onSubmit = (publish = true) =>
    handleSubmit(async ({ description, ...data }) => {
      ...
      const descContent = editorRef.current?.getContent()
      ...
      mutate(payload, {
        onSuccess: ({ product }) => {
          ...
          medusaClient.admin.client.request(
            "POST",
            `/admin/product/description/${product.id}`,
            { content: descContent }
          )
        },
        onError: (err) => {
        ...
        <FocusModal.Header>
          <div className="flex w-full justify-between px-8 medium:w-8/12">
            ...
            <div className="flex gap-x-small">
              ...
              >
                {/* Publish product */}
                {isLoading.current ? <Spinner /> : "Publish product"}
              </Button>
            </div>
          </div>
        </FocusModal.Header>
        <FocusModal.Main className="no-scrollbar flex w-full justify-center">
          <div className="my-16 max-w-[700px] small:w-4/5 medium:w-7/12 large:w-6/12">
            <Accordion defaultValue={["general"]} type="multiple">
              ...
              <Accordion.Item title="Description" value="description">
                <NewDescription
                  form={nestedForm(form, "description")}
                  editorRef={editorRef}
...
const createPayload = (
  data: Omit<NewProductForm, "description">,
  publish = true,
